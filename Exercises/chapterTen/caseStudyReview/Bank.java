...
public class Bank {
    ...
    * pre: matching account ID
    * post: An existing account has been modified.
    */
    public void modifyAccount(String acctID, Scanner input) {
        int index = 0;
        String getAcct = "";
        ...
        do {
            cust = (Account) accounts.get(index);
            getAcct = cust.getID();
            index++;
        } while (!(acctID.equals(getAcct)) && (index < accounts.size()));

        System.out.println("[DEBUG] get: " + getAcct + ", Acct: " + acctID);

        if (acctID.equals(getAcct)) {
            ...
            cust.changeAddress(street, city, state, zip);
        }
        else {
            System.out.println("No records found of ID: " + acctID);
        }
    }
